/**
 * Created by megan on 2017-04-17.
 */
...
exports.start = function(){
    var self = this;
    megan.log.notice('start Merge Overseer.....');
...
exports.stop = function(){
    megan.log.notice('pause Merge Overseer.....');
    clearInterval(this._interval);
};

// 새로운 워커 감시 시스템( Type C )
exports.merge = function(){
    megan.log.info('doing merge overseer..');
...
exports.requestSpotInstances = function(){
        ...
        if(!err){   // 프로시져 에러 없음
            if(rows[0][0].result == 0){ // 쿼리 성공
                ...
                if(jobCnt > wInstanceCnt){
                    // scale up
                    megan.log.notice('Merge Spot Scale Up............');

                    // 가능 서버(m - t)
                    // 대기잡(ts)
                    spotCnt = conf.overseer_v2.merge.max - tInstanceCnt;
                    ...
                    // scale down
                    megan.log.notice('Merge Spot Scale Down............');
    ...
    function fn_describeSpotPriceHistory(callback){
        ...
        var spot = {};
        spot.spotPrice = conf.overseer_v2.merge.price;
    ...
    function fn_createTags(instanceId){
        var tagParams = {
            ...
            Tags: [{
                Key: "Name",
                Value: conf.overseer_v2.merge.tag
            }]
        };
