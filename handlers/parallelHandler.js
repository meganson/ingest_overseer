/**
 * Created by megan on 2017-02-21.
 */
...
exports.init = function(){
    ...
    this._workerType = 'PT';
    this._instanceType = [];
    // this._instanceType.push('c4.8xlarge');
    this._instanceType.push('c4.4xlarge');  // 사양 다운
...
exports.start = function(){
    var self = this;
    megan.log.notice('start Parallel Overseer.....');
...
exports.stop = function(){
    megan.log.notice('pause Parallel Overseer.....');
    clearInterval(this._interval);
};

// 새로운 워커 감시 시스템( Type C )
exports.parallel = function(){
    megan.log.info('doing parallel overseer..');
...
exports.requestSpotInstances = function(){
        ...
        if(!err){   // 프로시져 에러 없음
            if(rows[0][0].result == 0){ // 쿼리 성공
                ...
                if(jobCnt > wInstanceCnt){
                    // scale up
                    megan.log.notice('Parallel Spot Scale Up............');

                    // 가능 서버(m - t)
                    // 대기잡(ts)
                    spotCnt = conf.overseer_v2.parallel.max - tInstanceCnt;
                    if(spotCnt > jobCnt) spotCnt = jobCnt;
                    if(spotCnt > conf.daemon.onceSpotCnt) spotCnt = conf.daemon.onceSpotCnt;

                    if(spotCnt > 0){
                        // async.waterfall([
                        //     fn_describeSpotPriceHistory,
                        //     fn_requestSpotInstances,
                        //     fn_setInterval,
                        //     fn_describeSpotInstanceRequest
                        // ], function(err, result){
                        //     if(err) return megan.log.error('something is wrong!', err);
                        //     return megan.log.debug('Done');
                        // });
                    ...
                    // scale down
                    megan.log.notice('Parallel Spot Scale Down............');
    ...
    function fn_describeSpotPriceHistory(callback){
        ...
        var spot = {};
        spot.spotPrice = conf.overseer_v2.parallel.price;
    ...
    function fn_createTags(instanceId){
        var tagParams = {
            ...
            Tags: [{
                Key: "Name",
                Value: conf.overseer_v2.parallel.tag
            }]
        };
